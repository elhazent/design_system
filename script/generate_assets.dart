...
void main() {
  ...
  bufferMainAssets.writeln(
    "  static const _basePath = 'packages/design_system';\n",
  );
  ...
  File('$outputRoot/app_assets.dart')
      .writeAsStringSync(bufferMainAssets.toString());

  // ignore: avoid_print
  print('✅ All Assets Generated in lib/src/assets Successfully!');
}
...
void _generateSubClass({
  ...
  for (var path in targetPaths) {
    ...
    for (var file in files) {
      ...
      if (RegExp(r'^(icon|ic|img|animation)[-_][a-zA-Z0-9]+')
          .hasMatch(rawName)) {
        cleanName = rawName;
      } else {
        cleanName = rawName
