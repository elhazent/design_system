...
md = """# 1. 📦 Package Overview & Architecture
...
1. **Add File:** Place in `assets/images/png/` or `assets/icons/svg/`.
2. **Update AppAssets:** Map in `lib/src/assets/app_assets.dart`:
```dart
static const myIcon = 'packages/design_system/assets/icons/svg/my_icon.svg';
```
3. **Usage:** `AppImage(asset: AppAssets.myIcon)` handles the package pathing automatically.
...
with open("PACKAGE_DOCS.md", "w") as f:
    f.write(md)
